...
async def get_rsvp(request: HttpRequest):
    try:
        ...
        all_events, rsvp, notifs = await asyncio.gather(sync_to_async(list)(Events.objects.values().order_by("date")), sync_to_async(list)(EventRsvp.objects.filter(user = user).values("event_id")), sync_to_async(list)(UserNotifications.objects.filter(user = user).values("event_id")))
